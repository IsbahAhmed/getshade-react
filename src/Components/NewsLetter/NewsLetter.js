  ...
  const [statusMsg,setStatusMsg] = useState({
   ...
   var handleSubmit = async (e)=>{
    ...
    if(inputValue){
      ...
      try{
        await firestore.collection('newsletter_subcriptions').doc(inputValue).set({email:inputValue})
    ...
    return (
      ...
      <form className="input-xs" onSubmit={handleSubmit}>
           ...
           {
             statusMsg.msg ?    <Paragraph style={{textAlign:"start",marginTop:"1rem",color:"var(--green)"}}>
            {
              statusMsg.msg
            }
           </Paragraph>
           : statusMsg.error ?    <Paragraph style={{textAlign:"start",marginTop:"1rem",color:"var(--red)"}}>
           {
             statusMsg.error
           }
         </Paragraph>
